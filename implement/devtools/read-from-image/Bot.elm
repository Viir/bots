{- This is a template to test functions to locate objects in images.
   ...
   The framework calls this function for every pixel in the image specified with the `--bot-settings`, and keeps a list of locations where `image_shows_object_at_origin` returned `True`.
...
type alias VolatileHostCreatedStructure =
    { processId : InterfaceToHost.VolatileProcessId }
            ...
            case taskResult of
                InterfaceToHost.CreateVolatileProcessResponse createVolatileHostResponse ->
                    case createVolatileHostResponse of
                        Err _ ->
                            { stateBefore | error = Just "Failed to create volatile process." }

                        Ok { processId } ->
                            { stateBefore | lastStepResult = VolatileHostSetupCompleted { processId = processId } }
        ...
        Initialized ->
            ContinueWithTask
                { task =
                    ...
                    , task = InterfaceToHost.CreateVolatileProcess { programCode = VolatileHostSetup.setupScript }
                    }
                    ...
                    task =
                        InterfaceToHost.RequestToVolatileProcess
                            (InterfaceToHost.RequestNotRequiringInputFocus
                                { processId = volatileHost.processId
